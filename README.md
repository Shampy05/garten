...
- **Reading Library**: A dedicated tab to find books in your target language (Google Books, with a keyless Open Library fallback), save them to a reading list, and track each one's status (want to read / reading / read), difficulty, notes, and dates — plus a per-language reading summary. Strict language filtering returns only books in the chosen language, and an ISBN quality gate keeps results to real published editions (no scanned public-domain documents or journals)
