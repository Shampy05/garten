        ...
        <div :class="isStale(item) ? 'opacity-60 transition-opacity hover:opacity-100' : ''">
          ...
          >
            <div
              ...
              :class="item.kind === 'new_language' || item.kind === 'reading_milestone' || item.kind === 'reading' || item.kind === 'vocab_mined'
            ...
            >
              ...
              <Sparkles v-else-if="item.kind === 'vocab_mined'" :size="16" />
            ...
            <div class="min-w-0 flex-1">
              <p class="text-sm text-stone-700 leading-snug">
                ...
                <template v-else-if="item.kind === 'vocab_mined'">
                  planted
                  <span class="font-medium text-garden-600">{{ item.details?.count || 0 }} new {{ (item.details?.count || 0) === 1 ? 'word' : 'words' }}</span>
                  from <span class="font-medium">{{ item.details?.book_title || 'a book' }}</span>
                  <span class="inline-flex items-center gap-1 align-baseline">
                    <span class="w-2 h-2 rounded-full inline-block" :style="{ backgroundColor: item.language_color || '#9ca3af' }"></span>
                    <span class="font-medium">{{ item.language_name || 'a language' }}</span>
                  </span>
              ...
              <div class="flex items-center justify-between mt-3">
                ...
                <button
                  ...
                  class="text-stone-300 hover:text-red-500 transition-colors sm:opacity-0 sm:group-hover:opacity-100"
...
import { Sprout, Trash2, Flower2, BookOpen, Sparkles } from 'lucide-vue-next'
