...
// "More by *author*" recommendation strip. Driven by the user's most recently
// finished book: if it has an author, search that author in the same language
// and return up to 6 unsaved books. The recommendation call uses the same
// merged pipeline so the user gets the full coverage, deduped.
//
// Pure helper: returns the candidate author + language (or null). The actual
// search is left to the caller so the recommendation row can render its own
// loading state.
export function recommendationSeed(savedBooks = []) {
  const finished = savedBooks
    .filter((b) => b.record?.status === 'read' && b.record?.finishedAt)
    .sort((a, b) => String(b.record.finishedAt).localeCompare(String(a.record.finishedAt)))
  for (const b of finished) {
    if (b.author && b.languageCode) {
      return { author: b.author, languageCode: b.languageCode, title: b.title }
    }
  }
  return null
}

export async function searchRecommendations({ savedBooks = [], pageSize = 8 } = {}) {
  const seed = recommendationSeed(savedBooks)
  if (!seed) return { books: [], seed: null }

  try {
    const { books } = await searchBooksMerged({
      query: seed.author,
      languageCode: seed.languageCode,
      page: 1,
      pageSize,
    })
    const savedIds = new Set(savedBooks.map((b) => b.externalId).filter(Boolean))
    // Drop the user's own saved copy of the same book so the strip is discovery,
    // not a re-display of the library.
    const filtered = books.filter((b) => !savedIds.has(b.externalId))
    return { books: filtered.slice(0, 6), seed }
  } catch (e) {
    return { books: [], seed }
  }
}
